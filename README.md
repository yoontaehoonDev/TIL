...
# 2021-08-03
  - DB Table에는 두 가지 관계가 존재하며,
    식별과 비식별 관계로 구분한다.
    식별 관계는 부모 테이블의 기본키가 자식 테이블에게는 기본키와 외래키로
    사용될 때 성립한다.
    비식별 관계는 부모 테이블의 기본키가 자식 테이블의 외래키로만 사용될 때 성립된다.
    여기서 큰 차이점은 비식별 관계의 경우, 부모 테이블이 없어도 자식 테이블이 생성이 가능하다는 것이다.
    반대로, 식별 관계에서는 자식 테이블은 부모 테이블이 없으면 생성이 될 수 없다.
    
    중요한 점은 비식별 관계에서 필수와 선택으로 구분된다.
    필수는 게시글과 댓글 테이블로 예를 들 수 있다.
    댓글을 작성하려면, 게시글이 존재해야 한다.
    그래서 게시글 PK는 댓글에 FK로 존재하지만, 댓글 고유 PK도 존재한다.
    이러한 관계를 필수적 비식별 관계라고 한다.
    외래키에 null을 허용하면 선택적 비식별 관계라 성립된다.


# 2021-08-04
  - IP Address Class
    - Class는 네트워크와 호스트의 영역을 나누는 역할을 한다.
      A부터 E까지 있지만, 네트워크와 호스트가 있는 A ~ C까지 다룬다.
      A의 네트워크 주소는 0 ~ 127이고, 호스트 주소는 0.0.0 ~ 255.255.255 이다.
      B의 네트워크 주소는 128.0 ~ 191.255이고, 호스트 주소는 0.0 ~ 255.255 이다.
      C의 네트워크 주소는 192.0.0 ~ 223.255.255이고, 호스트 주소는 0 ~ 255 이다.
      
      아래로 내려올수록 네트워크 주소 자릿수가 증가되지만, 호스트 주소는 감소된다.


# 2021-08-05
  - 시계열 데이터베이스
    - 관계형 데이터베이스는 Key와 Value 즉, 레코드와 필드로 이루어져 있다.
      반면에, 시계열 데이터베이스는 Time과 Value 즉, 시간과 값이 한쌍으로 이루어진다.
      실시간 데이터 처리를 용이하게 할 수 있는 것이 바로 시계열 데이터베이스이다.
