...
# 2021-04-19
  - 스프링 기초
    - 컨트롤러 / 서비스 / 리포지토리 / 도메인 의 의미
      - 컨트롤러 = 웹 MVC의 Controller Role
      - 서비스 = 핵심 비지니스 로직
      - 리포지토리 = DB에 접근하며 도메인 객체를 DB에 저장 및 관리
      - 도메인 = 비지니스 도메인 객체 Ex) 회원, 주문, 상품 등
                 주로 DB에 저장하고 관리
      
    - Optional<> 기능
      - 조회중에 값이 찾지 못 하면, null을 리턴한다.
      ```
    ...
    - findAny()
      ...
      store.values().stream()
        ...
        .findAny();
      ```
      member의 name이 파라미터의 name과 같으면, findAny() 메소드를
      통해 리턴한다.
      반면에, 일치하지 않더라도 null을 리턴한다.

      `store.values()` <- 멤버의 값들을 의미한다.

    - @AfterEach
      - 콜백 메소드이며, 메소드가 끝날 때마다 호출하는 애노테이션이다.
      
      - Map 객체에서 clear() 메소드를 추가하면,
        기존에 있던 값들을 전부 비운다.
    
    - save / findByName / findAll 테스트 코드
    ```
    @Test
    public void save() {
      Member member = new Member();
    ...
    그리고 Member 객체의 result에 repository.findById 메소드를
    파라미터값 member.getId() 을 넣고 호출한다.
    
    findById 메소드 내부에서 전달받은 id 파라미터를 가지고,
    Optional.ofNullable 메소드를 호출한다.
    id와 일치한 값이 존재하면, result는 그 값으로 치환되고,
    일치한 값이 없으면, result는 null로 치환된다.

    member중에 result와 일치하는 값이 있는지 assertThat을
    통해 검증한다.
    값이 존재하면 테스트는 통과하고,
    없으면 오류가 발생한다.
    ...
    ```
    Member 인스턴스를 생성하고,
    name 필드에 m1, m2 문자열을 넣는다.
    그리고 save 메소드를 통해 값들을 저장한다.
    result에 repository.findByName("m1").get()을 치환하고,
    m1 value가 없으면 result의 값은 null이고,
    있으면 파라미터 값 그대로 치환된다.

    그리고 m의 값과 result의 값이 같은지 assertThat을 통해 검증하고,
    있으면 테스트는 통과한다.

    여기서 주의 깊게 볼 부분은 assertThat(값) 이다.
    m이 m2로 바뀐다면, 테스트는 실패한다.
    왜냐하면, m2의 값에는 m1값이 존재하지 않기 때문이다.
    ...
    저장까진 위 코드들과 동일하다.
    List에 값들을 넣는다.
    여기서 2번 들어갔으므로, list.size()는 2가 된다.
    그래서 isEqualTo(2)와 일치한다.
    따라서 테스트는 통과한다.

    여기서 @AfterEach 가 필요하다.
    테스트 코드는 순서대로 진행을 보장하지 않는다.
    그래서 만일을 대비하여, @AfterEach를 사용하는 게 좋다.
    
    @AfterEach를 사용하지 않는 상황을 가정해보면,
    맨 처음 save() 가 실행되면,
    repository에는 하나의 값이 추가된 상태이다.
    그 상태로 findAll()로 넘어가서 실행하면
    save()에서 1번, findAll()에서 2번
    즉, list.size() 는 2가 아닌 3이 된다.
    따라서, findAll()의 isEqualTo(2)와 일치하지 않게 되고,
    오류를 초래한다.
    이러한 이유 때문에 @AfterEach 가 필요하다.
>>>>>>> 748d31840167b3deb1c4c1af0bb6ff9ea47424b1
